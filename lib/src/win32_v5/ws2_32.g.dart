...
/// ```
/// {@category winsock}
int GetAddrInfo(PWSTR? pNodeName, PWSTR? pServiceName,
...
final _GetAddrInfo = _ws2_32.lookupFunction<
    Int32 Function(PWSTR pNodeName, PWSTR pServiceName,
        Pointer<ADDRINFO> pHints, Pointer<Pointer<ADDRINFO>> ppResult),
    int Function(PWSTR pNodeName, PWSTR pServiceName, Pointer<ADDRINFO> pHints,
...
/// ```
/// {@category winsock}
Pointer<HOSTENT> gethostbyaddr(PSTR addr, int len, int type) =>
...
final _gethostbyaddr = _ws2_32.lookupFunction<
    ...
    Pointer<HOSTENT> Function(PSTR addr, int len, int type)>('gethostbyaddr');
...
/// ```
/// {@category winsock}
Pointer<HOSTENT> gethostbyname(PSTR name) => _gethostbyname(name);
...
/// ```
/// {@category winsock}
int gethostname(PSTR name, int namelen) => _gethostname(name, namelen);
...
/// ```
/// {@category winsock}
Pointer<PROTOENT> getprotobyname(PSTR name) => _getprotobyname(name);
...
/// ```
/// {@category winsock}
Pointer<SERVENT> getservbyname(PSTR name, PSTR? proto) =>
...
/// ```
/// {@category winsock}
Pointer<SERVENT> getservbyport(int port, PSTR? proto) =>
...
/// {@category winsock}
int getsockopt(
        int s, int level, int optname, PSTR optval, Pointer<Int32> optlen) =>
...
/// ```
/// {@category winsock}
int inet_addr(PSTR cp) => _inet_addr(cp);

final _inet_addr =
    _ws2_32.lookupFunction<Uint32 Function(PSTR cp), int Function(PSTR cp)>(
        'inet_addr');
...
/// ```
/// {@category winsock}
PSTR inet_ntoa(IN_ADDR in_) => _inet_ntoa(in_);

final _inet_ntoa = _ws2_32.lookupFunction<PSTR Function(IN_ADDR in_),
...
/// ```
/// {@category winsock}
PSTR inet_ntop(int family, Pointer pAddr, PSTR pStringBuf, int stringBufSize) =>
    _inet_ntop(family, pAddr, pStringBuf, stringBufSize);

final _inet_ntop = _ws2_32.lookupFunction<
    PSTR Function(
        Int32 family, Pointer pAddr, PSTR pStringBuf, IntPtr stringBufSize),
...
/// ```
/// {@category winsock}
int recv(int s, PSTR buf, int len, int flags) => _recv(s, buf, len, flags);
...
/// ```
/// {@category winsock}
int recvfrom(int s, PSTR buf, int len, int flags, Pointer<SOCKADDR>? from,
...
final _recvfrom = _ws2_32.lookupFunction<
    ...
    int Function(int s, PSTR buf, int len, int flags, Pointer<SOCKADDR> from,
        Pointer<Int32> fromlen)>('recvfrom');
...
/// ```
/// {@category winsock}
int send(int s, PSTR buf, int len, int flags) => _send(s, buf, len, flags);
...
/// {@category winsock}
int sendto(
        int s, PSTR buf, int len, int flags, Pointer<SOCKADDR> to, int tolen) =>
    _sendto(s, buf, len, flags, to, tolen);

final _sendto = _ws2_32.lookupFunction<
    Int32 Function(SOCKET s, PSTR buf, Int32 len, Int32 flags,
        Pointer<SOCKADDR> to, Int32 tolen),
    int Function(int s, PSTR buf, int len, int flags, Pointer<SOCKADDR> to,
        int tolen)>('sendto');
